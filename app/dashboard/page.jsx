  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      <main className="flex-1 px-4 py-6 sm:px-6 lg:px-8 pb-24 md:pb-6">
        ...
        {activeTab === 'community' && <CommunityFeed user={user} />}
        {activeTab === 'submit' && <IdeaSubmission user={user} />}
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t flex justify-around p-2 z-50">
        <MobileNavItem label="Community" icon={<Users className="h-5 w-5" />} onClick={() => setActiveTab('community')} active={activeTab === 'community'} />
        <MobileNavItem label="Submit" icon={<Plus className="h-5 w-5" />} onClick={() => setActiveTab('submit')} active={activeTab === 'submit'} />
        <MobileNavItem label="Profile" icon={<User className="h-5 w-5" />} onClick={() => setActiveTab('profile')} active={activeTab === 'profile'} />
        <MobileNavItem label="Settings" icon={<Settings className="h-5 w-5" />} onClick={() => setActiveTab('settings')} active={activeTab === 'settings'} />
      </nav>
    </div>
  );
}
...
function MobileNavItem({ label, icon, onClick, active }) {
  return (
    <button
      onClick={onClick}
      className={`flex flex-col items-center space-y-1 p-2 rounded-md text-xs font-medium ${
